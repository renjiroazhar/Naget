          ...
          <Grid item xs={12}>
            <FormControl className={classes.formControl}>
              ...
              >
                ...
                {values.kecamatan === 'Banyumanik'
                  ? banyumanik &&
                    banyumanik.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Candisari'
                  ? candisari &&
                    candisari.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Gajahmungkur'
                  ? gajahmungkur &&
                    gajahmungkur.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Gayamsari'
                  ? gayamsari &&
                    gayamsari.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Genuk'
                  ? genuk &&
                    genuk.map(data => <MenuItem value={data}>{data}</MenuItem>)
                  : values.kecamatan === 'Gunungpati'
                  ? gunungpati &&
                    gunungpati.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  ...
                  : values.kecamatan === 'Mijen'
                  ? mijen &&
                    mijen.map(data => <MenuItem value={data}>{data}</MenuItem>)
                  : values.kecamatan === 'Ngaliyan'
                  ? ngaliyan &&
                    ngaliyan.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Pedurungan'
                  ? pedurungan &&
                    pedurungan.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Semarang Barat'
                  ? semarang_barat &&
                    semarang_barat.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Semarang Selatan'
                  ? semarang_selatan &&
                    semarang_selatan.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Semarang Tengah'
                  ? semarang_tengah &&
                    semarang_tengah.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Semarang Timur'
                  ? semarang_timur &&
                    semarang_timur.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Semarang Utara'
                  ? semarang_utara &&
                    semarang_utara.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Tembalang'
                  ? tembalang &&
                    tembalang.map(data => (
                      <MenuItem value={data}>{data}</MenuItem>
                    ))
                  : values.kecamatan === 'Tugu'
                  ? tugu &&
                    tugu.map(data => <MenuItem value={data}>{data}</MenuItem>)
                  : null}
              </Select>
            </FormControl>
          </Grid>
